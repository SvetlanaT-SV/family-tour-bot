...
def _sanitize_html_for_telegram(text: str) -> str:
    ...
    #
    # _strip_banned_words           — отключено (резало прилагательные → дыры)
    # _trim_description_to_one_sentence — отключено (обрезало посреди абзаца)
    # _strip_broken_trailing_advantage  — отключено (удаляло ✅-пункты целиком)
    #
    # Что оставляем — только безопасные правки:
    # 1) Дубль-строка между заголовком и деталями (не ломает грамматику)
    text = _strip_intro_between_headline_and_details(text)
    # 2) Стилистика: точки в конце строк (косметика для соцсетей)
    text = re.sub(r"\.+[ \t]*$", "", text, flags=re.MULTILINE)
    # 3) Пустая строка перед хэштегами
    text = re.sub(r"([^\n])\n(#[A-Za-zА-Яа-яЁё])", r"\1\n\n\2", text)
    return text
...
1. ЗАГОЛОВОК в <b>...</b> с одной эмодзи в начале — короткий, цепкий, продающий.
   — 5-9 слов, ОДНА строка.
   — Желательно одна цифра: цена ({price_str}), количество ночей ({nights_str}), скидка или количество мест.
   — Обращение на «вы».
   — Без штампов: «ГОРЯЩИЙ ТУР!», «Спешите», «Лучший отдых», «Незабываемый отпуск», «Райский уголок».
   — Без повторов: «успейте до пятницы», «до конца недели», «не упустите».
   — Без markdown-разделителей `---`, `***`.
   — НЕ дублируй детали ниже — заголовок ≠ повторение блока деталей.
...
3. ОПИСАНИЕ — 1-2 коротких предложения о фактах (где отель, что важно туристу).
   — Используй конкретику из КОНТЕКСТА ОБ ОТЕЛЕ (если есть) или общеизвестные факты о курорте/стране.
   — Избегай шаблонных эпитетов: «уютный», «комфортный», «роскошный», «незабываемый», «прекрасный», «волшебный», «сказочный», «райский».
   — Избегай пустых фраз: «вас ждёт», «погрузитесь в атмосферу», «откройте для себя», «массу впечатлений», «душой и телом».
...
4. 2 преимущества — каждое отдельной строкой, начинается с ✅.
   — Каждое ✅ — конкретная выгода с цифрой или фактом (а не «✅ Отличный сервис»).
   — Преимущества РАЗНЫЕ — не повторяй один и тот же угол дважды (например цена + цена).
   — Перед ✅ пунктами НЕ ставь подзаголовки типа «Особенности:» или «Преимущества:».
   — Используй факты из КОНТЕКСТА ОБ ОТЕЛЕ если есть, иначе общие сильные стороны (прямой вылет, питание, длительность, сезон, цена).
