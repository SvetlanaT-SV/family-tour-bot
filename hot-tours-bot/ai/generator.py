...
def _sanitize_html_for_telegram(text: str) -> str:
    ...
    text = re.sub(
        r"^\s*[^\w]*\s*(Заголовок|Уникальные особенности отеля|Особенности отеля|Особенности"
        r"|Преимущества(?:\s+тура)?|Где отдыхаем|Куда едем|Как связаться|Контакты|Связаться с нами"
        r"|Что включено|Чем хорош этот отель|Почему стоит выбрать|О тур[еa])\s*:?\s*$",
        "",
        text,
        flags=re.MULTILINE | re.IGNORECASE,
    )
    ...
    # 3) Точки в конце строк (косметика для соцсетей)
...
Вот ровно такая структура поста (просто СГЕНЕРИРУЙ ВЕСЬ ТЕКСТ ниже, заменяя то что в [квадратных скобках]; всё остальное копируй один в один; квадратные скобки в результате не сохраняй):

<b>[короткий цепкий заголовок 5-9 слов с одной эмодзи в начале, желательно с одной цифрой — ценой {price_str}, или ночами {nights_str}, или скидкой; обращение на «вы»; без штампов «ГОРЯЩИЙ ТУР!», «Спешите», «Незабываемый отпуск», «Райский уголок», «успейте до пятницы», «не упустите»]</b>
...
[Одно или два коротких предложения о фактах: где отель, что важно туристу. Используй данные из КОНТЕКСТА ОБ ОТЕЛЕ если он подан; иначе общие известные факты о курорте/стране. Избегай слов «уютный», «комфортный», «роскошный», «незабываемый», «прекрасный», «волшебный», «сказочный», «райский». Избегай фраз «вас ждёт», «погрузитесь в атмосферу», «откройте для себя», «массу впечатлений», «душой и телом».]

✅ [Конкретная выгода с цифрой или фактом, не общая похвала.]
✅ [Вторая выгода ДРУГОГО типа чем первая — если первая про цену, вторая про вылет, питание или особенность отеля.]

Примеры ХОРОШИХ преимуществ (для подражания, не копировать дословно):
   • «✅ 6 ночей {meal_ru.lower()} за {price_str} — это на 30% дешевле обычной цены сезона»
   • «✅ Прямой вылет из {city_gen} — без пересадок и долгой дороги»
   • «✅ 50 метров до песчаного пляжа — выход прямо из отеля»
   • «✅ Аквапарк и 4 ресторана в стоимости — отдыхайте без лишних трат»
   • «✅ Утренний рейс — обед уже на курорте»

Примеры ПЛОХИХ преимуществ (так писать НЕ нужно):
   • «✅ Выгодная цена на весь тур» — нет цифры, общо
   • «✅ Удобство расположения рядом с морем» — без цифр и фактов
   • «✅ Качественный сервис» — пусто
   • «✅ Отличный отдых для семьи» — клише
...
ВАЖНО:
— НЕ пиши слова «Заголовок:», «Где отдыхаем:», «Преимущества тура:», «Как связаться:», «Описание:» и любые другие подобные подзаголовки секций. Сразу пиши содержимое.
— Каждая строка деталей (✈️/🏨/🍽/💰) ОБЯЗАТЕЛЬНА.
— Между блоками — пустая строка.
— HTML только <b>...</b>, никаких <br>/<p>/<div>/**markdown**.
