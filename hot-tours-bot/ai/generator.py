...
def _sanitize_html_for_telegram(text: str) -> str:
    ...
    # — это служебные мета-заголовки которые модель иногда ставит перед самим постом
    text = re.sub(r"^\s*#{1,6}\s+.*$", "", text, flags=re.MULTILINE)
    # И отдельные "preamble"-строки типа "Продающий пост для Telegram", "Текст поста:"
    text = re.sub(
        r"^\s*(Продающий пост|Текст поста|Пост для (Telegram|Тг|ВК|канала)|Готовый пост)\s*[:.]?\s*$",
        "",
        text,
        flags=re.MULTILINE | re.IGNORECASE,
    )
...
3. ОДНО короткое предложение (15-25 слов МАКС, не больше). НИКАКИХ двух абзацев. НИКАКОГО продолжения через переноса строки.
   — Если есть «Особенности отеля» — выбери 1-2 самых сильных факта (расстояние до моря / линия / тип питания / спа / детская инфраструктура) и слепи в одно предложение.
   — Если особенностей нет — одна короткая фраза про сезон/курорт ({resort or country}) с привязкой к датам ({date}).
   — ЗАПРЕЩЁННЫЕ СЛОВА (это вода, в посте быть НЕ ДОЛЖНО ни в каком виде): роскошный, роскошь, великолепный, великолепие, незабываемый, незабываемые впечатления, комфортный, комфортная атмосфера, уютный, уют, прекрасный, удивительный, волшебный, сказочный, отдых мечты, мечты сбываются, райский, рай, дивный, восхитительный, чарующий, прелестный, очаровательный.
   — ЗАПРЕЩЁННЫЕ ФРАЗЫ (даже если без запрещённых слов): «вас ждёт», «вы погрузитесь», «насладитесь», «получите массу впечатлений», «подарит вам ... атмосферу», «впечатления благодаря», «приятное дополнение», «расслабьтесь», «откройте для себя», «всё что нужно для», «всё для вашего отдыха».
   — ХОРОШИЙ пример: «Отель в центре Нячанга, 200 метров до пляжа, утренний чай-кофе включён.»
   — ПЛОХОЙ пример: «Во Вьетнаме вас ждёт уютный отель в самом сердце Нячанга, расположенный в нескольких минутах ходьбы от достопримечательностей! Отдых здесь подарит вам комфортную атмосферу и незабываемые впечатления.» — слишком длинно, есть «вас ждёт», «уютный», «комфортную», «незабываемые».
