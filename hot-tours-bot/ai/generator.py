...
# Шаблонные обороты — заменяем фразу полностью.
# Используем [ \t]+ вместо \s+ — чтобы переносы строк остались на месте.
# ВАЖНО: глаголы вроде «наслад\w+» НЕ режем программно (ломает грамматику
# окружающих слов). Только короткие шаблоны, после удаления которых
# предложение остаётся читаемым.
_BANNED_PHRASES = (
...
# Существительные-«вода»: «комфорт», «уют», «роскошь» — могут быть
# в любом падеже («комфорта», «роскоши»). Режем вместе с предшествующим
# союзом «и »/«или » и/или ведущей запятой, чтобы не оставлять висящих.
_BANNED_NOUN_ROOTS = (
    r"комфорт",   # комфорт, комфорта, комфорту
    r"уют",       # уют, уюта, уюту
    r"роскош",    # роскошь, роскоши, роскошью
)
...
def _strip_banned_words(text: str) -> str:
    ...
    if not text:
        return text
    # 1. Прилагательные перед существительным: «<корень>+окончание<пробел>».
    for root in _BANNED_ADJECTIVE_ROOTS:
        text = re.sub(rf"\b{root}\w+[ \t]+", "", text, flags=re.IGNORECASE)
    # 2. Существительные («комфорта», «уюта», «роскоши»).
    #    Стараемся захватить вместе с ведущим союзом «и/или» и/или запятой,
    #    чтобы не оставить «X, Y и .» с висящим союзом.
    for root in _BANNED_NOUN_ROOTS:
        # Случай "X, Y и <банное_слово>." → "X, Y."
        text = re.sub(
            rf"[ \t]*,?[ \t]+(?:и|или)[ \t]+{root}\w*\b",
            "",
            text,
        ...
        # Случай "<банное_слово> ..." как отдельное слово
        text = re.sub(rf"\b{root}\w*[ \t]+", "", text, flags=re.IGNORECASE)
    # 3. Шаблонные фразы
    for pattern, repl in _BANNED_PHRASES:
        text = re.sub(pattern, repl, text, flags=re.IGNORECASE)
    # 4. Подчищаем мусор после удалений:
    #    «X, , Y» → «X, Y»
    text = re.sub(r"\s*,\s*,\s*", ", ", text)
    #    «слово , .» → «слово.» (висящая запятая перед точкой)
    text = re.sub(r"\s+,\s*([.!?])", r"\1", text)
    #    « — .» → «.» (висящее тире перед точкой)
    text = re.sub(r"\s+—\s*([.!?])", r"\1", text)
    #    Двойные пробелы и табы (НЕ \n) → одинарный пробел
    text = re.sub(r"[ \t]{2,}", " ", text)
    #    Лишние пробелы перед знаком препинания
...
def _sanitize_html_for_telegram(text: str) -> str:
    ...
    # Удаляем markdown-разделители "---" / "***" / "___" (3 и более одного символа)
    ...
    # Удаляем inline-ярлыки в начале строки: "Описание:", "Описание отеля:", "Резюме:".
    # Это НЕ удаляет содержимое строки — только сам ярлык.
    text = re.sub(
        r"^[ \t]*[«\"']?(Описание(?:[ \t]+отеля)?|Резюме|Краткое описание|"
        r"О направлении|Об отеле|Кратко об отеле|Информация об отеле)[«\"']?[ \t]*:[ \t]*",
        "",
        text,
        flags=re.MULTILINE | re.IGNORECASE,
    )
    ...
    # 2) Между заголовком (первой <b>-строкой) и блоком деталей (✈️ Вылет)
    #    не должно быть текста — модель иногда добавляет «подзаголовок»-расшифровку.
    text = _strip_intro_between_headline_and_details(text)
    # 3) Обрезаем блок-описание до одного предложения
...
def _strip_intro_between_headline_and_details(text: str) -> str:
    """
    Если между заголовком (первая жирная строка <b>...</b>) и блоком деталей
    (строка начинающаяся с ✈️ Вылет) есть «лишняя» текстовая строка —
    удаляем её. Это убирает дубль-описание типа «Выезд из Уфы 19 мая, 6 ночей...»,
    которое модель иногда вставляет под заголовком.
    """
    if not text or "<b>" not in text or "✈️" not in text:
        return text
    lines = text.split("\n")
    # Индекс первой строки с <b>...</b> (заголовок)
    headline_idx = None
    for i, line in enumerate(lines):
        if "<b>" in line and "</b>" in line and headline_idx is None:
            headline_idx = i
            break
    if headline_idx is None:
        return text
    # Индекс первой строки с ✈️ ПОСЛЕ заголовка
    details_idx = None
    for j in range(headline_idx + 1, len(lines)):
        if lines[j].strip().startswith("✈️"):
            details_idx = j
            break
    if details_idx is None:
        return text
    # Всё что между заголовком и деталями (кроме пустых строк) — удаляем
    new_lines = lines[: headline_idx + 1] + [""] + lines[details_idx:]
    return "\n".join(new_lines)
...
def generate_post_from_dict(data: dict, api_key: str = "") -> str:
    ...
    # Рандомизация стилей — чтобы посты не повторялись по фразам
    headline_style = random.choice([
        "вопрос с интригой (например: «А вы знали, что Турция в мае дешевле Сочи?»)",
        "конкретная выгода + цифра (например: «38 900 ₽ за 7 ночей у моря»)",
        "срочность с дедлайном — но НЕ «успейте до пятницы» (например: «До конца недели — Хургада 5⭐ −35%»)",
        "сравнение (например: «Турция или Египет за 45К? Выбирайте»)",
        "восклицание-приглашение (например: «Открыли прямой рейс Уфа → Бодрум!»)",
        "FOMO про места (например: «Осталось 4 места: Шарм-эш-Шейх, 7 ночей ультра»)",
        "новость про сезон (например: «Майские в Турции по апрельским ценам»)",
        "цифра в начале (например: «49 800 ₽ — и ты в Дубае на 5 ночей»)",
    ])

    adv_angles = random.sample([
        "цена против рынка (на сколько дешевле обычного, в %)",
        "конкретный факт отеля (расстояние до моря, размер номера, аквапарк — что-то из КОНТЕКСТА)",
        "вылет (день недели/время вылета, прямой/без пересадок, удобство расписания)",
        "питание (что включено, особенность кухни, режим работы ресторанов)",
        "длительность отдыха (соотношение цена/количество ночей)",
        "сезон (почему именно сейчас — нет толпы / лучшая погода / низкие цены)",
        "семейные удобства (мини-клуб, детский бассейн, питание для детей)",
        "пляж и активности (тип пляжа, что включено бесплатно, спорт/анимация)",
    ], k=2)
...
1. ЗАГОЛОВОК в <b>...</b> с одной эмодзи в начале — должен **жечь и продавать**. Правила:
   ...
   — ⛔ Запрещены повторяющиеся фразы из прошлых постов: «успейте до пятницы», «до конца недели», «не упустите», «спешите забронировать».
   — ⛔ Заголовок — ОДНА СТРОКА. После него СРАЗУ пустая строка, потом блок деталей с ✈️ Вылет. НЕ добавляй вторую строку под заголовком в стиле «Выезд из Уфы 19 мая, 6 ночей в отеле X» — это дубль того что уже будет в блоке деталей.
   — НЕ используй markdown-разделители «---», «***» — это не нужно в посте.

   🎯 **СТИЛЬ ЗАГОЛОВКА ДЛЯ ЭТОГО ПОСТА** (используй именно этот стиль, не комбинируй):
   {headline_style}

   Не копируй пример дословно — используй ЭТОТ стиль, но СВОИМИ словами и СВОИМИ деталями (на основе данных тура).
...
4. РОВНО 2 ПРЕИМУЩЕСТВА. Каждое — отдельной строкой, начинается с ✅.
   — ⛔ КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО: ставить ПЕРЕД ними подзаголовки вроде «🌊 Особенности отеля:», «📍 Преимущества:», «Что включено:», «Чем хорош отель:», или ЛЮБОЙ другой строки заканчивающейся двоеточием. Сразу после описания — пустая строка, потом ✅, потом ✅.
   — ⛔ КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО: писать факты об отеле списком БЕЗ ✅.

   🎯 **УГЛЫ ПРЕИМУЩЕСТВ ДЛЯ ЭТОГО ПОСТА** (используй ровно эти два, в этом порядке):
   1) {adv_angles[0]}
   2) {adv_angles[1]}

   — Каждое ✅ — это **конкретная выгода с цифрой или фактом**, не общая похвала.
   — ⛔ Запрещены повторяющиеся шаблоны из прошлых постов: «Прямой вылет из Уфы — без пересадок в Москве», «Все включено за {price_str}», «без пересадок».
   — ❌ ПЛОХО: «✅ Качественный сервис», «✅ Прекрасное расположение», «✅ Отличный отдых для семьи».
   — ✅ ХОРОШО (для разных углов):
     • цена против рынка: «✅ 7 ночей за 38К — дешевле, чем неделя в Сочи»
     • факт отеля: «✅ 50 м до песчаного пляжа, спуск прямо из отеля»
     • вылет: «✅ Утренний рейс — на курорте уже к обеду»
     • питание: «✅ 4 ресторана à la carte + бар у бассейна 24/7»
     • сезон: «✅ Май — вода 24°, ещё нет толпы и низкие цены»
     • семья: «✅ Мини-клуб 4-12 лет и детский бассейн с горками»
     • пляж: «✅ Собственный песчаный пляж, шезлонги и зонтики бесплатно»
