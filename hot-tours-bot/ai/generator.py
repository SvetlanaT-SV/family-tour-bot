...
def _random_headline(country: str, hotel: str, nights: str,
    ...
    """
    country_acc = _country_acc(country)  # винительный падеж: "в Турцию"
    templates = [
        ...
        f"💥 {hotel} в {country_acc} — вот это цена!",
        f"🤩 Мечтали о {country}? Вот ваш шанс!",
        f"⚡ {nights} в {country_acc} — и это реально!",
        f"🏖 {country}: отдых мечты за разумные деньги",
        f"🌊 Море, солнце, {meal_ru} — {country} ждёт!",
        f"😍 {hotel} {stars_str} в {country_acc} — по цене, которую вы не ожидали",
        f"🎯 Идеальный вариант найден: {country}, {hotel}",
        f"🚀 Собирайте чемодан! Горящий тур в {country_acc}",
        f"💫 Лучшая цена этой недели — {country} от {price_str}/чел",
        f"🌟 {country}: {nights} {meal_ru.lower()} — по такой цене надолго не задержится",
        f"🏝 Хотите в отпуск? {country} — это реально!",
        f"🔑 Ваш билет на море: {hotel}, {country}",
        f"👀 Смотрите, что мы нашли: {country}, {hotel} {stars_str}",
...
1. Первая строка — цепляющий заголовок в тегах <b>...</b>. Он должен быть уникальным, вызывать желание читать дальше и купить тур. НЕ используй шаблонное «🔥 ГОРЯЩИЙ ТУР!» — придумай что-то живое. Например: «😍 Мечтали о {country}? Вот ваш шанс!» или «⚡ {hotel} — и это горящая цена!» или «🌊 Море, солнце, {meal_ru} — {country} ждёт!»
