...
_BANNED_PHRASES = (
    (r"\bнаслажд\w+\s+", ""),                 # "наслаждаемся X" → " X"
    (r"\bпогрузит\w+\s+в\s+атмосфер\w+\s*",
     ""),                                       # "погрузитесь в атмосферу"
    (r"\bвас\s+ждёт\s+",        ""),           # "вас ждёт"
    (r"\bвас\s+ждет\s+",        ""),
    (r"\bдушой\s+и\s+телом\s+", ""),
    (r"\bмассу\s+впечатлений\s*", ""),
    (r"\bоткройте\s+для\s+себя\s+", ""),
    (r"\bподарит\s+\w+\s+атмосфер\w+\s*", ""),  # "подарит X атмосферу"
    # подмены «родной город» → правильный город указывается ниже в _post_substitute_city
)
...
def _strip_banned_words(text: str) -> str:
    ...
    for root in _BANNED_ADJECTIVE_ROOTS:
        text = re.sub(rf"\b{root}\w+\s+", "", text, flags=re.IGNORECASE)
    # Шаблонные фразы
    for pattern, repl in _BANNED_PHRASES:
        text = re.sub(pattern, repl, text, flags=re.IGNORECASE)
    # Подчищаем двойные пробелы и пустоты перед знаками препинания
